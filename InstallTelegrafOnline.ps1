...
$zipPath = Join-Path -Path $destino -ChildPath "telegraf-1.35.3_windows_amd64.zip"
wget https://dl.influxdata.com/telegraf/releases/telegraf-1.35.3_windows_amd64.zip -UseBasicParsing -OutFile $zipPath
...
Move-Item -Path (Join-Path -Path $destino -ChildPath "telegraf-1.35.3\telegraf.*") -Destination $destino

# Eliminar el archivo ZIP y el directorio descomprimido
Remove-Item -Path $zipPath
Remove-Item -Path (Join-Path -Path $destino -ChildPath "telegraf-1.35.3") -Recurse -Force
